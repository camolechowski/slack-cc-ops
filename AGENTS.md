...
- The Bash hook's only direct Docker lifecycle authority is the two exact cloudflared sidecar restarts plus `cd /win && docker compose -p win-live|win-staging up|stop|rm ...`. Compose `up` is start-only: it requires `-d --no-build --no-recreate --no-deps --pull never` and an explicit known service. Other container/project names, Compose file overrides, direct `stop`/`rm`, and Compose `down`/`restart` remain denied.
