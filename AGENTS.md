...
- The Bash hook's only direct Docker lifecycle authority is the two exact cloudflared sidecar restarts plus `cd /win && docker compose -p win-live|win-staging stop|start <known-service>`. Compose `start` acts only on existing containers. Compose `up` and `rm` are always denied because creation/recreation from on-disk config is operator-gated; a wedged container that needs `rm` + `up` escalates to a human. Other container/project names, Compose file overrides, direct `stop`/`rm`, and Compose `down`/`restart` remain denied.
