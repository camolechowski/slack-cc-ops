...
# SLACK_CC_OPS_DIR. From Cam's laptop: ssh superpea@mac-mini 'bash ~/dvl/win-ops/scripts/redeploy.sh'
...
log "3/6 docker compose up -d --build --force-recreate slack-cc-ops"
HOST_DOCKER_GID="$(stat -c '%g' /var/run/docker.sock 2>/dev/null || stat -f '%g' /var/run/docker.sock 2>/dev/null || echo 1)" \
  docker compose up -d --build --force-recreate slack-cc-ops 2>&1 | tail -8

log "4/6 waiting for container to be up"
sleep 12

log "5/6 dismissing dev-channel warning prompt (sends Enter to tmux)"
docker exec slack-cc-ops tmux send-keys -t slackcc Enter 2>/dev/null || log "  (tmux not ready yet — entrypoint may still be sleeping; try again or check 'docker logs slack-cc-ops')"
sleep 8
...
log "Verify in Slack with: @win-ops what's the dev status?"
